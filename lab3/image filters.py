...
"")
    ...
    def check_selected_item(self, text):
        if text == "Web Cam":
            self.rdbtnSort.setChecked(True)
            self.video_path=""
            self.capture_video(10);
            self.show_message_dialog("Your Webcame is recorded you can find it in same directory . thank You:)))");
    ...
    # Create a black image (canvas) as a background
        img = 255 * np.ones((300, 600, 3), dtype=np.uint8)

        # Put the message on the image
        cv2.putText(img, message, (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

        # Display the image
        cv2.imshow('Message Dialog', img)
    ...
    def capture_video(self,duration):
        cap = cv2.VideoCapture(0)  # Open the webcam
        fourcc = cv2.VideoWriter_fourcc(*'XVID')  # Codec for saving the video
        out = cv2.VideoWriter('captured_video.avi', fourcc, 20.0, (640, 480))  # Video writer object
        ...
        cap.release()  # Release the webcam
        out.release()  # Release the video writers
        cv2.destroyAllWindows() 
